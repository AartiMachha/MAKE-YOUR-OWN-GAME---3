...
function setup() {
  ...
  ring.x = Math.round(random(473, 670));
  ring.scale = 0.1;
  ...
  notes.scale = 0.1;
  notes.setCollider("rectangle", 0,0, 37,37);
...
function draw() {
 ...
 if(keyWentDown(DOWN_ARROW)){
  ...
  if(thief.isTouching(key1) && thief.isTouching(treasure)){
    treasures += 190;
  }
  ...
  if(thief.isTouching(notes) || thief.x < 200 || thief.y < 200){
    notes.remove();
    key1.visible = true;
    score = score + 5;
  }
...
function diamondF(){
  ...
  diamond.x = Math.round(random(473,670));
